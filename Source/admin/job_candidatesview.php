...
$job_candidates_view = NULL; // Initialize page object first

class cjob_candidates_view extends cjob_candidates {
	...
	// Page object name
	var $PageObjName = 'job_candidates_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "job_candidateslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "job_candidateslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($job_candidates_view)) $job_candidates_view = new cjob_candidates_view();

// Page init
$job_candidates_view->Page_Init();

// Page main
$job_candidates_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$job_candidates_view->Page_Render();
?>
...
// Page object
var job_candidates_view = new ew_Page("job_candidates_view");
job_candidates_view.PageID = "view"; // Page ID
var EW_PAGE_ID = job_candidates_view.PageID; // For backward compatibility

// Form object
var fjob_candidatesview = new ew_Form("fjob_candidatesview");

// Form_CustomValidate event
fjob_candidatesview.Form_CustomValidate = 
...
fjob_candidatesview.ValidateRequired = true;
<?php } else { ?>
fjob_candidatesview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fjob_candidatesview.Lists["x_job_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_title_en","x_title_ar","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php $job_candidates_view->ExportOptions->Render("body") ?>
<?php
	foreach ($job_candidates_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($job_candidates->Export == "") { ?>
...
<?php } ?>
<?php $job_candidates_view->ShowPageHeader(); ?>
<?php
$job_candidates_view->ShowMessage();
?>
<?php if ($job_candidates->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($job_candidates_view->Pager)) $job_candidates_view->Pager = new cNumericPager($job_candidates_view->StartRec, $job_candidates_view->DisplayRecs, $job_candidates_view->TotalRecs, $job_candidates_view->RecRange) ?>
<?php if ($job_candidates_view->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($job_candidates_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $job_candidates_view->PageUrl() ?>start=<?php echo $job_candidates_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($job_candidates_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $job_candidates_view->PageUrl() ?>start=<?php echo $job_candidates_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($job_candidates_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $job_candidates_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($job_candidates_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $job_candidates_view->PageUrl() ?>start=<?php echo $job_candidates_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($job_candidates_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $job_candidates_view->PageUrl() ?>start=<?php echo $job_candidates_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<form name="fjob_candidatesview" id="fjob_candidatesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($job_candidates_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $job_candidates_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="job_candidates">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($job_candidates->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_job_candidates_id"><?php echo $job_candidates->id->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->id->CellAttributes() ?>>
<span id="el_job_candidates_id" class="form-group">
<span<?php echo $job_candidates->id->ViewAttributes() ?>>
<?php echo $job_candidates->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($job_candidates->job_id->Visible) { // job_id ?>
	<tr id="r_job_id">
		<td><span id="elh_job_candidates_job_id"><?php echo $job_candidates->job_id->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->job_id->CellAttributes() ?>>
<span id="el_job_candidates_job_id" class="form-group">
<span<?php echo $job_candidates->job_id->ViewAttributes() ?>>
<?php echo $job_candidates->job_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($job_candidates->name->Visible) { // name ?>
	<tr id="r_name">
		<td><span id="elh_job_candidates_name"><?php echo $job_candidates->name->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->name->CellAttributes() ?>>
<span id="el_job_candidates_name" class="form-group">
<span<?php echo $job_candidates->name->ViewAttributes() ?>>
<?php echo $job_candidates->name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($job_candidates->_email->Visible) { // email ?>
	<tr id="r__email">
		<td><span id="elh_job_candidates__email"><?php echo $job_candidates->_email->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->_email->CellAttributes() ?>>
<span id="el_job_candidates__email" class="form-group">
<span<?php echo $job_candidates->_email->ViewAttributes() ?>>
<?php echo $job_candidates->_email->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($job_candidates->mobile->Visible) { // mobile ?>
	<tr id="r_mobile">
		<td><span id="elh_job_candidates_mobile"><?php echo $job_candidates->mobile->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->mobile->CellAttributes() ?>>
<span id="el_job_candidates_mobile" class="form-group">
<span<?php echo $job_candidates->mobile->ViewAttributes() ?>>
<?php echo $job_candidates->mobile->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($job_candidates->cv->Visible) { // cv ?>
	<tr id="r_cv">
		<td><span id="elh_job_candidates_cv"><?php echo $job_candidates->cv->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->cv->CellAttributes() ?>>
<span id="el_job_candidates_cv" class="form-group">
...
<a<?php echo $job_candidates->cv->LinkAttributes() ?>><?php echo $job_candidates->cv->ViewValue ?></a>
<?php } elseif (!in_array($job_candidates->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($job_candidates->cv->Upload->DbValue)) { ?>
<?php echo $job_candidates->cv->ViewValue ?>
<?php } elseif (!in_array($job_candidates->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php if ($job_candidates->applied_date->Visible) { // applied_date ?>
	<tr id="r_applied_date">
		<td><span id="elh_job_candidates_applied_date"><?php echo $job_candidates->applied_date->FldCaption() ?></span></td>
		<td<?php echo $job_candidates->applied_date->CellAttributes() ?>>
<span id="el_job_candidates_applied_date" class="form-group">
<span<?php echo $job_candidates->applied_date->ViewAttributes() ?>>
<?php echo $job_candidates->applied_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fjob_candidatesview.Init();
</script>
<?php
$job_candidates_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($job_candidates->Export == "") { ?>
...
<?php
$job_candidates_view->Page_Terminate();
?>
