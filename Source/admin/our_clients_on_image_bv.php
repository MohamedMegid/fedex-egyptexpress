...
$our_clients_on_image_blobview = NULL; // Initialize page object first

class cour_clients_on_image_blobview extends cour_clients {
	...
	// Page object name
	var $PageObjName = 'our_clients_on_image_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('our_clients', 'x_on_image');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('on_image');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('on_image')));
				if (trim(strval($this->Recordset->fields('on_image'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('on_image') . "\"");
				}
...
// Create page object
if (!isset($our_clients_on_image_blobview)) $our_clients_on_image_blobview = new cour_clients_on_image_blobview();

// Page init
$our_clients_on_image_blobview->Page_Init();

// Page main
$our_clients_on_image_blobview->Page_Main();
?>
<?php
$our_clients_on_image_blobview->Page_Terminate();
?>
