...
$cms_add = NULL; // Initialize page object first

class ccms_add extends ccms {
	...
	function LoadDefaultValues() {
		...
		$this->content_en->CurrentValue = NULL;
		$this->content_en->OldValue = $this->content_en->CurrentValue;
		$this->content_ar->CurrentValue = NULL;
		$this->content_ar->OldValue = $this->content_ar->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// content_en
		$this->content_en->SetDbValueDef($rsnew, $this->content_en->CurrentValue, "", FALSE);

		// content_ar
		$this->content_ar->SetDbValueDef($rsnew, $this->content_ar->CurrentValue, "", FALSE);
		if (!$this->image->Upload->KeepFile) {
			$this->image->UploadPath = '../webroot/uploads/images/';
...
// Create page object
if (!isset($cms_add)) $cms_add = new ccms_add();
...
// Page object
var cms_add = new ew_Page("cms_add");
cms_add.PageID = "add"; // Page ID
var EW_PAGE_ID = cms_add.PageID; // For backward compatibility

// Form object
var fcmsadd = new ew_Form("fcmsadd");

// Validate form
fcmsadd.Validate = function() {
...
fcmsadd.ValidateRequired = true;
<?php } else { ?>
fcmsadd.ValidateRequired = false; 
<?php } ?>
...
<?php $cms_add->ShowPageHeader(); ?>
<?php
$cms_add->ShowMessage();
?>
<form name="fcmsadd" id="fcmsadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cms_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cms_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fcmsadd", "x_content_en", 60, 6, <?php echo ($cms->content_en->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
ew_CreateEditor("fcmsadd", "x_content_ar", 60, 6, <?php echo ($cms->content_ar->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
