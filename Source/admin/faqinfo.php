...
class cfaq extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('faq', 'faq', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// question_en
		$this->question_en = new cField('faq', 'faq', 'x_question_en', 'question_en', '`question_en`', '`question_en`', 200, -1, FALSE, '`question_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['question_en'] = &$this->question_en;

		// question_ar
		$this->question_ar = new cField('faq', 'faq', 'x_question_ar', 'question_ar', '`question_ar`', '`question_ar`', 200, -1, FALSE, '`question_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['question_ar'] = &$this->question_ar;

		// answer_en
		$this->answer_en = new cField('faq', 'faq', 'x_answer_en', 'answer_en', '`answer_en`', '`answer_en`', 201, -1, FALSE, '`answer_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['answer_en'] = &$this->answer_en;

		// answer_ar
		$this->answer_ar = new cField('faq', 'faq', 'x_answer_ar', 'answer_ar', '`answer_ar`', '`answer_ar`', 201, -1, FALSE, '`answer_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['answer_ar'] = &$this->answer_ar;

		// last_modified
		$this->last_modified = new cField('faq', 'faq', 'x_last_modified', 'last_modified', '`last_modified`', 'DATE_FORMAT(`last_modified`, \'%d/%m/%Y\')', 135, 7, FALSE, '`last_modified`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`faq`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("faqview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("faqview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "faqadd.php?" . $this->UrlParm($parm);
		else
			return "faqadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("faqedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("faqadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("faqdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->question_en->Exportable) $Doc->ExportCaption($this->question_en);
					if ($this->question_ar->Exportable) $Doc->ExportCaption($this->question_ar);
					if ($this->answer_en->Exportable) $Doc->ExportCaption($this->answer_en);
					if ($this->answer_ar->Exportable) $Doc->ExportCaption($this->answer_ar);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->question_en->Exportable) $Doc->ExportField($this->question_en);
						if ($this->question_ar->Exportable) $Doc->ExportField($this->question_ar);
						if ($this->answer_en->Exportable) $Doc->ExportField($this->answer_en);
						if ($this->answer_ar->Exportable) $Doc->ExportField($this->answer_ar);
