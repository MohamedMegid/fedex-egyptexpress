...
class ccms extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('cms', 'cms', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// image
		$this->image = new cField('cms', 'cms', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['image'] = &$this->image;

		// title_en
		$this->title_en = new cField('cms', 'cms', 'x_title_en', 'title_en', '`title_en`', '`title_en`', 200, -1, FALSE, '`title_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title_en'] = &$this->title_en;

		// title_ar
		$this->title_ar = new cField('cms', 'cms', 'x_title_ar', 'title_ar', '`title_ar`', '`title_ar`', 200, -1, FALSE, '`title_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title_ar'] = &$this->title_ar;

		// content_en
		$this->content_en = new cField('cms', 'cms', 'x_content_en', 'content_en', '`content_en`', '`content_en`', 201, -1, FALSE, '`content_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['content_en'] = &$this->content_en;

		// content_ar
		$this->content_ar = new cField('cms', 'cms', 'x_content_ar', 'content_ar', '`content_ar`', '`content_ar`', 201, -1, FALSE, '`content_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['content_ar'] = &$this->content_ar;

		// last_modified
		$this->last_modified = new cField('cms', 'cms', 'x_last_modified', 'last_modified', '`last_modified`', 'DATE_FORMAT(`last_modified`, \'%d/%m/%Y\')', 135, 7, FALSE, '`last_modified`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cms`";
	}
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "cmsadd.php?" . $this->UrlParm($parm);
		else
			return "cmsadd.php";
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->content_en->Exportable) $Doc->ExportCaption($this->content_en);
					if ($this->content_ar->Exportable) $Doc->ExportCaption($this->content_ar);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->content_en->Exportable) $Doc->ExportField($this->content_en);
						if ($this->content_ar->Exportable) $Doc->ExportField($this->content_ar);
