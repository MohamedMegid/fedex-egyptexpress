...
$accounts_avatar_blobview = NULL; // Initialize page object first

class caccounts_avatar_blobview extends caccounts {
	...
	// Page object name
	var $PageObjName = 'accounts_avatar_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('accounts', 'x_avatar');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('avatar');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('avatar')));
				if (trim(strval($this->Recordset->fields('avatar'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('avatar') . "\"");
				}
...
// Create page object
if (!isset($accounts_avatar_blobview)) $accounts_avatar_blobview = new caccounts_avatar_blobview();

// Page init
$accounts_avatar_blobview->Page_Init();

// Page main
$accounts_avatar_blobview->Page_Main();
?>
<?php
$accounts_avatar_blobview->Page_Terminate();
?>
