...
//
// Table class for our_clients
//
class cour_clients extends cTable {
	var $id;
	var $off_image;
	var $on_image;
	var $created;
	...
	function __construct() {
		...
		$this->TableVar = 'our_clients';
		$this->TableName = 'our_clients';
		...
		// id
		$this->id = new cField('our_clients', 'our_clients', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// off_image
		$this->off_image = new cField('our_clients', 'our_clients', 'x_off_image', 'off_image', '`off_image`', '`off_image`', 200, -1, TRUE, '`off_image`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['off_image'] = &$this->off_image;

		// on_image
		$this->on_image = new cField('our_clients', 'our_clients', 'x_on_image', 'on_image', '`on_image`', '`on_image`', 200, -1, TRUE, '`on_image`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['on_image'] = &$this->on_image;

		// created
		$this->created = new cField('our_clients', 'our_clients', 'x_created', 'created', '`created`', 'DATE_FORMAT(`created`, \'%d/%m/%Y\')', 135, 7, FALSE, '`created`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`our_clients`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("our_clientsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("our_clientsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "our_clientsadd.php?" . $this->UrlParm($parm);
		else
			return "our_clientsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("our_clientsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("our_clientsadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("our_clientsdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->off_image->Exportable) $Doc->ExportCaption($this->off_image);
					if ($this->on_image->Exportable) $Doc->ExportCaption($this->on_image);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->off_image->Exportable) $Doc->ExportField($this->off_image);
						if ($this->on_image->Exportable) $Doc->ExportField($this->on_image);
