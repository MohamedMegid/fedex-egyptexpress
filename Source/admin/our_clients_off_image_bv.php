...
$our_clients_off_image_blobview = NULL; // Initialize page object first

class cour_clients_off_image_blobview extends cour_clients {
	...
	// Page object name
	var $PageObjName = 'our_clients_off_image_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'our_clients');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('our_clients', 'x_off_image');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('off_image');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('off_image')));
				if (trim(strval($this->Recordset->fields('off_image'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('off_image') . "\"");
				}
...
// Create page object
if (!isset($our_clients_off_image_blobview)) $our_clients_off_image_blobview = new cour_clients_off_image_blobview();

// Page init
$our_clients_off_image_blobview->Page_Init();

// Page main
$our_clients_off_image_blobview->Page_Main();
?>
<?php
$our_clients_off_image_blobview->Page_Terminate();
?>
