...
class caccounts_edit extends caccounts {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->commercial_register);
...
// Page object
var accounts_edit = new ew_Page("accounts_edit");
accounts_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = accounts_edit.PageID; // For backward compatibility

// Form object
var faccountsedit = new ew_Form("faccountsedit");
...
<?php if (!isset($accounts_edit->Pager)) $accounts_edit->Pager = new cNumericPager($accounts_edit->StartRec, $accounts_edit->DisplayRecs, $accounts_edit->TotalRecs, $accounts_edit->RecRange) ?>
<?php if ($accounts_edit->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($accounts_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $accounts_edit->PageUrl() ?>start=<?php echo $accounts_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($accounts_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $accounts_edit->PageUrl() ?>start=<?php echo $accounts_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($accounts_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $accounts_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($accounts_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $accounts_edit->PageUrl() ?>start=<?php echo $accounts_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($accounts_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $accounts_edit->PageUrl() ?>start=<?php echo $accounts_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<form name="faccountsedit" id="faccountsedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php if ($accounts->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_accounts_id" class="col-sm-2 control-label ewLabel"><?php echo $accounts->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $accounts->id->CellAttributes() ?>>
<span id="el_accounts_id">
<span<?php echo $accounts->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $accounts->id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($accounts->id->CurrentValue) ?>">
<?php echo $accounts->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_commercial_register"] == "0") { ?>
<input type="hidden" name="fa_x_commercial_register" id= "fa_x_commercial_register" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_commercial_register" id= "fa_x_commercial_register" value="1">
<?php } ?>
