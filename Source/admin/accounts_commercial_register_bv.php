...
$accounts_commercial_register_blobview = NULL; // Initialize page object first

class caccounts_commercial_register_blobview extends caccounts {
	...
	// Page object name
	var $PageObjName = 'accounts_commercial_register_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'accounts');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('accounts', 'x_commercial_register');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('commercial_register');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('commercial_register')));
				if (trim(strval($this->Recordset->fields('commercial_register'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('commercial_register') . "\"");
				}
...
// Create page object
if (!isset($accounts_commercial_register_blobview)) $accounts_commercial_register_blobview = new caccounts_commercial_register_blobview();

// Page init
$accounts_commercial_register_blobview->Page_Init();

// Page main
$accounts_commercial_register_blobview->Page_Main();
?>
<?php
$accounts_commercial_register_blobview->Page_Terminate();
?>
