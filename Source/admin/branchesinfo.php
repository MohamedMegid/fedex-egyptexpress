...
class cbranches extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('branches', 'branches', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// title_en
		$this->title_en = new cField('branches', 'branches', 'x_title_en', 'title_en', '`title_en`', '`title_en`', 200, -1, FALSE, '`title_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title_en'] = &$this->title_en;

		// title_ar
		$this->title_ar = new cField('branches', 'branches', 'x_title_ar', 'title_ar', '`title_ar`', '`title_ar`', 200, -1, FALSE, '`title_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title_ar'] = &$this->title_ar;

		// lat
		$this->lat = new cField('branches', 'branches', 'x_lat', 'lat', '`lat`', '`lat`', 200, -1, FALSE, '`lat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->lat->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['lat'] = &$this->lat;

		// lng
		$this->lng = new cField('branches', 'branches', 'x_lng', 'lng', '`lng`', '`lng`', 200, -1, FALSE, '`lng`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->lng->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['lng'] = &$this->lng;

		// text_en
		$this->text_en = new cField('branches', 'branches', 'x_text_en', 'text_en', '`text_en`', '`text_en`', 201, -1, FALSE, '`text_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['text_en'] = &$this->text_en;

		// text_ar
		$this->text_ar = new cField('branches', 'branches', 'x_text_ar', 'text_ar', '`text_ar`', '`text_ar`', 201, -1, FALSE, '`text_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['text_ar'] = &$this->text_ar;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("branchesview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("branchesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "branchesadd.php?" . $this->UrlParm($parm);
		else
			return "branchesadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("branchesedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("branchesadd.php", $this->UrlParm($parm));
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->text_en->Exportable) $Doc->ExportCaption($this->text_en);
					if ($this->text_ar->Exportable) $Doc->ExportCaption($this->text_ar);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->title_en->Exportable) $Doc->ExportCaption($this->title_en);
					if ($this->title_ar->Exportable) $Doc->ExportCaption($this->title_ar);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->text_en->Exportable) $Doc->ExportField($this->text_en);
						if ($this->text_ar->Exportable) $Doc->ExportField($this->text_ar);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->title_en->Exportable) $Doc->ExportField($this->title_en);
						if ($this->title_ar->Exportable) $Doc->ExportField($this->title_ar);
