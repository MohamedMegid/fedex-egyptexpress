...
//
// Table class for integra_status
//
class cintegra_status extends cTable {
	var $id;
	var $code_id;
	var $code_name_en;
	var $code_name_ar;
	...
	function __construct() {
		...
		$this->TableVar = 'integra_status';
		$this->TableName = 'integra_status';
		...
		// id
		$this->id = new cField('integra_status', 'integra_status', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// code_id
		$this->code_id = new cField('integra_status', 'integra_status', 'x_code_id', 'code_id', '`code_id`', '`code_id`', 200, -1, FALSE, '`code_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['code_id'] = &$this->code_id;

		// code_name_en
		$this->code_name_en = new cField('integra_status', 'integra_status', 'x_code_name_en', 'code_name_en', '`code_name_en`', '`code_name_en`', 200, -1, FALSE, '`code_name_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['code_name_en'] = &$this->code_name_en;

		// code_name_ar
		$this->code_name_ar = new cField('integra_status', 'integra_status', 'x_code_name_ar', 'code_name_ar', '`code_name_ar`', '`code_name_ar`', 200, -1, FALSE, '`code_name_ar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['code_name_ar'] = &$this->code_name_ar;

		// status
		$this->status = new cField('integra_status', 'integra_status', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['status'] = &$this->status;

		// last_modified
		$this->last_modified = new cField('integra_status', 'integra_status', 'x_last_modified', 'last_modified', '`last_modified`', 'DATE_FORMAT(`last_modified`, \'%d/%m/%Y\')', 135, 7, FALSE, '`last_modified`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`integra_status`";
	}
	...
	// Update Table
	var $UpdateTable = "`integra_status`";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("integra_statusview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("integra_statusview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "integra_statusadd.php?" . $this->UrlParm($parm);
		else
			return "integra_statusadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("integra_statusedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("integra_statusadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("integra_statusdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->code_id->Exportable) $Doc->ExportCaption($this->code_id);
					if ($this->code_name_en->Exportable) $Doc->ExportCaption($this->code_name_en);
					if ($this->code_name_ar->Exportable) $Doc->ExportCaption($this->code_name_ar);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->code_id->Exportable) $Doc->ExportField($this->code_id);
						if ($this->code_name_en->Exportable) $Doc->ExportField($this->code_name_en);
						if ($this->code_name_ar->Exportable) $Doc->ExportField($this->code_name_ar);
