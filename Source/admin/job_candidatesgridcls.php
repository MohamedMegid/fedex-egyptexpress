...
$job_candidates_grid = NULL; // Initialize page object first

class cjob_candidates_grid extends cjob_candidates {
	...
	// Page object name
	var $PageObjName = 'job_candidates_grid';

	// Grid form hidden field names
	var $FormName = 'fjob_candidatesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["job_candidates"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_job_id") && $objForm->HasValue("o_job_id") && $this->job_id->CurrentValue <> $this->job_id->OldValue)
			return FALSE;
		...
		if (!ew_Empty($this->cv->Upload->Value))
			return FALSE;
		if ($objForm->HasValue("x_applied_date") && $objForm->HasValue("o_applied_date") && $this->applied_date->CurrentValue <> $this->applied_date->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->job_id->setOldValue($objForm->GetValue("o_job_id"));
		...
		$this->applied_date->setOldValue($objForm->GetValue("o_applied_date"));
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (is_numeric($this->RowIndex) && !$this->EventCancelled) ew_RenderUploadField($this->cv, $this->RowIndex);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "jobs") {
			...
			if ($GLOBALS["jobs"]->EventCancelled) $this->EventCancelled = TRUE;
		}
