...
//
class ctranslation extends cTable {
	var $pk_id;
	var $id;
	var $english;
	var $arabic;
	...
	function __construct() {
		...
		// pk_id
		$this->pk_id = new cField('translation', 'translation', 'x_pk_id', 'pk_id', '`pk_id`', '`pk_id`', 19, -1, FALSE, '`pk_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->pk_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['pk_id'] = &$this->pk_id;

		// id
		$this->id = new cField('translation', 'translation', 'x_id', 'id', '`id`', '`id`', 200, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['id'] = &$this->id;

		// english
		$this->english = new cField('translation', 'translation', 'x_english', 'english', '`english`', '`english`', 201, -1, FALSE, '`english`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['english'] = &$this->english;

		// arabic
		$this->arabic = new cField('translation', 'translation', 'x_arabic', 'arabic', '`arabic`', '`arabic`', 201, -1, FALSE, '`arabic`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['arabic'] = &$this->arabic;

		// last_modified
		$this->last_modified = new cField('translation', 'translation', 'x_last_modified', 'last_modified', '`last_modified`', 'DATE_FORMAT(`last_modified`, \'%d/%m/%Y\')', 135, 7, FALSE, '`last_modified`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`translation`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('pk_id', $rs))
				ew_AddFilter($where, ew_QuotedName('pk_id') . '=' . ew_QuotedValue($rs['pk_id'], $this->pk_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->pk_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@pk_id@", ew_AdjustSql($this->pk_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("translationview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("translationview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "translationadd.php?" . $this->UrlParm($parm);
		else
			return "translationadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("translationedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("translationadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("translationdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->pk_id->CurrentValue)) {
			$sUrl .= "pk_id=" . urlencode($this->pk_id->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->pk_id->Exportable) $Doc->ExportCaption($this->pk_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->pk_id->Exportable) $Doc->ExportField($this->pk_id);
