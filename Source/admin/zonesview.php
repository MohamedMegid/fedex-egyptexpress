...
$zones_view = NULL; // Initialize page object first

class czones_view extends czones {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "zoneslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "zoneslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_zones\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_zones',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fzonesview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($zones_view)) $zones_view = new czones_view();

// Page init
$zones_view->Page_Init();

// Page main
$zones_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zones_view->Page_Render();
?>
...
// Page object
var zones_view = new ew_Page("zones_view");
zones_view.PageID = "view"; // Page ID
var EW_PAGE_ID = zones_view.PageID; // For backward compatibility

// Form object
var fzonesview = new ew_Form("fzonesview");

// Form_CustomValidate event
fzonesview.Form_CustomValidate = 
...
fzonesview.ValidateRequired = true;
<?php } else { ?>
fzonesview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $zones_view->ExportOptions->Render("body") ?>
<?php
	foreach ($zones_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($zones->Export == "") { ?>
...
<?php } ?>
<?php $zones_view->ShowPageHeader(); ?>
<?php
$zones_view->ShowMessage();
?>
<?php if ($zones->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($zones_view->Pager)) $zones_view->Pager = new cNumericPager($zones_view->StartRec, $zones_view->DisplayRecs, $zones_view->TotalRecs, $zones_view->RecRange) ?>
<?php if ($zones_view->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($zones_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $zones_view->PageUrl() ?>start=<?php echo $zones_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($zones_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $zones_view->PageUrl() ?>start=<?php echo $zones_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($zones_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $zones_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($zones_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $zones_view->PageUrl() ?>start=<?php echo $zones_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($zones_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $zones_view->PageUrl() ?>start=<?php echo $zones_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<form name="fzonesview" id="fzonesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zones_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zones_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="zones">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($zones->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_zones_id"><?php echo $zones->id->FldCaption() ?></span></td>
		<td<?php echo $zones->id->CellAttributes() ?>>
<span id="el_zones_id" class="form-group">
<span<?php echo $zones->id->ViewAttributes() ?>>
<?php echo $zones->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones->city_en->Visible) { // city_en ?>
	<tr id="r_city_en">
		<td><span id="elh_zones_city_en"><?php echo $zones->city_en->FldCaption() ?></span></td>
		<td<?php echo $zones->city_en->CellAttributes() ?>>
<span id="el_zones_city_en" class="form-group">
<span<?php echo $zones->city_en->ViewAttributes() ?>>
<?php echo $zones->city_en->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones->city_ar->Visible) { // city_ar ?>
	<tr id="r_city_ar">
		<td><span id="elh_zones_city_ar"><?php echo $zones->city_ar->FldCaption() ?></span></td>
		<td<?php echo $zones->city_ar->CellAttributes() ?>>
<span id="el_zones_city_ar" class="form-group">
<span<?php echo $zones->city_ar->ViewAttributes() ?>>
<?php echo $zones->city_ar->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones->zone->Visible) { // zone ?>
	<tr id="r_zone">
		<td><span id="elh_zones_zone"><?php echo $zones->zone->FldCaption() ?></span></td>
		<td<?php echo $zones->zone->CellAttributes() ?>>
<span id="el_zones_zone" class="form-group">
<span<?php echo $zones->zone->ViewAttributes() ?>>
<?php echo $zones->zone->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones->last_modified->Visible) { // last_modified ?>
	<tr id="r_last_modified">
		<td><span id="elh_zones_last_modified"><?php echo $zones->last_modified->FldCaption() ?></span></td>
		<td<?php echo $zones->last_modified->CellAttributes() ?>>
<span id="el_zones_last_modified" class="form-group">
<span<?php echo $zones->last_modified->ViewAttributes() ?>>
<?php echo $zones->last_modified->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
