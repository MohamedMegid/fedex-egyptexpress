...
$our_clients_view = NULL; // Initialize page object first

class cour_clients_view extends cour_clients {
	...
	// Page object name
	var $PageObjName = 'our_clients_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "our_clientslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "our_clientslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($our_clients_view)) $our_clients_view = new cour_clients_view();

// Page init
$our_clients_view->Page_Init();

// Page main
$our_clients_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$our_clients_view->Page_Render();
?>
...
// Page object
var our_clients_view = new ew_Page("our_clients_view");
our_clients_view.PageID = "view"; // Page ID
var EW_PAGE_ID = our_clients_view.PageID; // For backward compatibility

// Form object
var four_clientsview = new ew_Form("four_clientsview");

// Form_CustomValidate event
four_clientsview.Form_CustomValidate = 
...
four_clientsview.ValidateRequired = true;
<?php } else { ?>
four_clientsview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $our_clients_view->ExportOptions->Render("body") ?>
<?php
	foreach ($our_clients_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($our_clients->Export == "") { ?>
...
<?php } ?>
<?php $our_clients_view->ShowPageHeader(); ?>
<?php
$our_clients_view->ShowMessage();
?>
<?php if ($our_clients->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($our_clients_view->Pager)) $our_clients_view->Pager = new cNumericPager($our_clients_view->StartRec, $our_clients_view->DisplayRecs, $our_clients_view->TotalRecs, $our_clients_view->RecRange) ?>
<?php if ($our_clients_view->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($our_clients_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $our_clients_view->PageUrl() ?>start=<?php echo $our_clients_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($our_clients_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $our_clients_view->PageUrl() ?>start=<?php echo $our_clients_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($our_clients_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $our_clients_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($our_clients_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $our_clients_view->PageUrl() ?>start=<?php echo $our_clients_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($our_clients_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $our_clients_view->PageUrl() ?>start=<?php echo $our_clients_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<form name="four_clientsview" id="four_clientsview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($our_clients_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $our_clients_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="our_clients">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($our_clients->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_our_clients_id"><?php echo $our_clients->id->FldCaption() ?></span></td>
		<td<?php echo $our_clients->id->CellAttributes() ?>>
<span id="el_our_clients_id" class="form-group">
<span<?php echo $our_clients->id->ViewAttributes() ?>>
<?php echo $our_clients->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($our_clients->off_image->Visible) { // off_image ?>
	<tr id="r_off_image">
		<td><span id="elh_our_clients_off_image"><?php echo $our_clients->off_image->FldCaption() ?></span></td>
		<td<?php echo $our_clients->off_image->CellAttributes() ?>>
<span id="el_our_clients_off_image" class="form-group">
<span>
<?php echo ew_GetFileViewTag($our_clients->off_image, $our_clients->off_image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($our_clients->on_image->Visible) { // on_image ?>
	<tr id="r_on_image">
		<td><span id="elh_our_clients_on_image"><?php echo $our_clients->on_image->FldCaption() ?></span></td>
		<td<?php echo $our_clients->on_image->CellAttributes() ?>>
<span id="el_our_clients_on_image" class="form-group">
<span>
<?php echo ew_GetFileViewTag($our_clients->on_image, $our_clients->on_image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($our_clients->created->Visible) { // created ?>
	<tr id="r_created">
		<td><span id="elh_our_clients_created"><?php echo $our_clients->created->FldCaption() ?></span></td>
		<td<?php echo $our_clients->created->CellAttributes() ?>>
<span id="el_our_clients_created" class="form-group">
<span<?php echo $our_clients->created->ViewAttributes() ?>>
<?php echo $our_clients->created->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
four_clientsview.Init();
</script>
<?php
$our_clients_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($our_clients->Export == "") { ?>
...
<?php
$our_clients_view->Page_Terminate();
?>
