...
$zones_prices_view = NULL; // Initialize page object first

class czones_prices_view extends czones_prices {
	...
	// Page object name
	var $PageObjName = 'zones_prices_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["weight"] <> "") {
			$this->RecKey["weight"] = $_GET["weight"];
			$KeyUrl .= "&amp;weight=" . urlencode($this->RecKey["weight"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["weight"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["weight"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "zones_priceslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "zones_priceslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($zones_prices_view)) $zones_prices_view = new czones_prices_view();

// Page init
$zones_prices_view->Page_Init();

// Page main
$zones_prices_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zones_prices_view->Page_Render();
?>
...
// Page object
var zones_prices_view = new ew_Page("zones_prices_view");
zones_prices_view.PageID = "view"; // Page ID
var EW_PAGE_ID = zones_prices_view.PageID; // For backward compatibility

// Form object
var fzones_pricesview = new ew_Form("fzones_pricesview");

// Form_CustomValidate event
fzones_pricesview.Form_CustomValidate = 
...
fzones_pricesview.ValidateRequired = true;
<?php } else { ?>
fzones_pricesview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $zones_prices_view->ExportOptions->Render("body") ?>
<?php
	foreach ($zones_prices_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($zones_prices->Export == "") { ?>
...
<?php } ?>
<?php $zones_prices_view->ShowPageHeader(); ?>
<?php
$zones_prices_view->ShowMessage();
?>
<?php if ($zones_prices->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($zones_prices_view->Pager)) $zones_prices_view->Pager = new cNumericPager($zones_prices_view->StartRec, $zones_prices_view->DisplayRecs, $zones_prices_view->TotalRecs, $zones_prices_view->RecRange) ?>
<?php if ($zones_prices_view->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($zones_prices_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $zones_prices_view->PageUrl() ?>start=<?php echo $zones_prices_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($zones_prices_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $zones_prices_view->PageUrl() ?>start=<?php echo $zones_prices_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($zones_prices_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $zones_prices_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($zones_prices_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $zones_prices_view->PageUrl() ?>start=<?php echo $zones_prices_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($zones_prices_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $zones_prices_view->PageUrl() ?>start=<?php echo $zones_prices_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<form name="fzones_pricesview" id="fzones_pricesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zones_prices_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zones_prices_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="zones_prices">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($zones_prices->weight->Visible) { // weight ?>
	<tr id="r_weight">
		<td><span id="elh_zones_prices_weight"><?php echo $zones_prices->weight->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->weight->CellAttributes() ?>>
<span id="el_zones_prices_weight" class="form-group">
<span<?php echo $zones_prices->weight->ViewAttributes() ?>>
<?php echo $zones_prices->weight->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->zone1->Visible) { // zone1 ?>
	<tr id="r_zone1">
		<td><span id="elh_zones_prices_zone1"><?php echo $zones_prices->zone1->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->zone1->CellAttributes() ?>>
<span id="el_zones_prices_zone1" class="form-group">
<span<?php echo $zones_prices->zone1->ViewAttributes() ?>>
<?php echo $zones_prices->zone1->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->zone2->Visible) { // zone2 ?>
	<tr id="r_zone2">
		<td><span id="elh_zones_prices_zone2"><?php echo $zones_prices->zone2->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->zone2->CellAttributes() ?>>
<span id="el_zones_prices_zone2" class="form-group">
<span<?php echo $zones_prices->zone2->ViewAttributes() ?>>
<?php echo $zones_prices->zone2->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->zone3->Visible) { // zone3 ?>
	<tr id="r_zone3">
		<td><span id="elh_zones_prices_zone3"><?php echo $zones_prices->zone3->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->zone3->CellAttributes() ?>>
<span id="el_zones_prices_zone3" class="form-group">
<span<?php echo $zones_prices->zone3->ViewAttributes() ?>>
<?php echo $zones_prices->zone3->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->zone4->Visible) { // zone4 ?>
	<tr id="r_zone4">
		<td><span id="elh_zones_prices_zone4"><?php echo $zones_prices->zone4->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->zone4->CellAttributes() ?>>
<span id="el_zones_prices_zone4" class="form-group">
<span<?php echo $zones_prices->zone4->ViewAttributes() ?>>
<?php echo $zones_prices->zone4->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->zone5->Visible) { // zone5 ?>
	<tr id="r_zone5">
		<td><span id="elh_zones_prices_zone5"><?php echo $zones_prices->zone5->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->zone5->CellAttributes() ?>>
<span id="el_zones_prices_zone5" class="form-group">
<span<?php echo $zones_prices->zone5->ViewAttributes() ?>>
<?php echo $zones_prices->zone5->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->zone6->Visible) { // zone6 ?>
	<tr id="r_zone6">
		<td><span id="elh_zones_prices_zone6"><?php echo $zones_prices->zone6->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->zone6->CellAttributes() ?>>
<span id="el_zones_prices_zone6" class="form-group">
<span<?php echo $zones_prices->zone6->ViewAttributes() ?>>
<?php echo $zones_prices->zone6->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zones_prices->last_modified->Visible) { // last_modified ?>
	<tr id="r_last_modified">
		<td><span id="elh_zones_prices_last_modified"><?php echo $zones_prices->last_modified->FldCaption() ?></span></td>
		<td<?php echo $zones_prices->last_modified->CellAttributes() ?>>
<span id="el_zones_prices_last_modified" class="form-group">
<span<?php echo $zones_prices->last_modified->ViewAttributes() ?>>
<?php echo $zones_prices->last_modified->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fzones_pricesview.Init();
</script>
<?php
$zones_prices_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($zones_prices->Export == "") { ?>
...
<?php
$zones_prices_view->Page_Terminate();
?>
