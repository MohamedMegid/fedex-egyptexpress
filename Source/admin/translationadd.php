...
$translation_add = NULL; // Initialize page object first

class ctranslation_add extends ctranslation {
	...
	// Page object name
	var $PageObjName = 'translation_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["pk_id"] != "") {
				...
				$this->setKey("pk_id", $this->pk_id->CurrentValue); // Set up key
			} else {
				$this->setKey("pk_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("translationlist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "translationview.php")
	...
	function AddRow($rsold = NULL) {
		...
		// arabic
		$this->arabic->SetDbValueDef($rsnew, $this->arabic->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$this->pk_id->setDbValue($conn->Insert_ID());
			$rsnew['pk_id'] = $this->pk_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($translation_add)) $translation_add = new ctranslation_add();

// Page init
$translation_add->Page_Init();

// Page main
$translation_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$translation_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var translation_add = new ew_Page("translation_add");
translation_add.PageID = "add"; // Page ID
var EW_PAGE_ID = translation_add.PageID; // For backward compatibility

// Form object
var ftranslationadd = new ew_Form("ftranslationadd");

// Validate form
ftranslationadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $translation->id->FldCaption(), $translation->id->ReqErrMsg)) ?>");
...
ftranslationadd.Form_CustomValidate = 
...
ftranslationadd.ValidateRequired = true;
<?php } else { ?>
ftranslationadd.ValidateRequired = false; 
<?php } ?>
...
<?php $translation_add->ShowPageHeader(); ?>
<?php
$translation_add->ShowMessage();
?>
<form name="ftranslationadd" id="ftranslationadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($translation_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $translation_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="translation">
<input type="hidden" name="a_add" id="a_add" value="A">
<div>
<?php if ($translation->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_translation_id" for="x_id" class="col-sm-2 control-label ewLabel"><?php echo $translation->id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $translation->id->CellAttributes() ?>>
<span id="el_translation_id">
<input type="text" data-field="x_id" name="x_id" id="x_id" size="90" maxlength="255" placeholder="<?php echo ew_HtmlEncode($translation->id->PlaceHolder) ?>" value="<?php echo $translation->id->EditValue ?>"<?php echo $translation->id->EditAttributes() ?>>
</span>
<?php echo $translation->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ftranslationadd.Init();
</script>
<?php
$translation_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$translation_add->Page_Terminate();
?>
